...
language
  	...
  	code, default to `en`. The term "Anonymous" will be in the language you
  	set. If you use the default template, the template will also be in
  	the language. (You are welcome to contribute by adding the translated
    `comment_lang.plain` and `comment_lang.html` to `isso/templates`. The
    default `comment.plain` and `comment.html` in that directory are what
